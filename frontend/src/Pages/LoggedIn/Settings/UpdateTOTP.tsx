import { useState } from "react";
import { User } from "../../../App";
import Logo from "../../../Components/Logo";
import { FaCopy } from "react-icons/fa6";

// Page for user to enable or disable 2FA
function Update2FA(props: { dark: boolean; user: User; logout: () => void }) {
	// Initialise state
	const [totpCode, setTotpCode] = useState("");
	const [recoveryCode, setRecoveryCode] = useState("");
	const [password, setPassword] = useState("");
	const [error, setError] = useState("");
	const [message, setMessage] = useState("");
	const [downloaded, setDownloaded] = useState(false);
	const [copied, setCopied] = useState(false);
	const [tfaConfig, setTfaConfig] = useState<{
		secret: string;
		image: string;
		recovery_codes: string[];
	}>();
	const disable2FA = () => {
		// disable 2FA using a PUT request
		setError("Please wait...");
		fetch(
			`/api/user/${props.user.user_id}/2fa?password=${password}&${
				totpCode ? `totp_code=${totpCode}` : `recovery_code=${recoveryCode}`
			}`,
			{
				method: "PUT",
				body: JSON.stringify({
					tfa_enabled: false,
					totp_secret: ""
				})
			}
		)
			.then((r) => r.json())
			.then((r) => {
				if (r.error) {
					switch (r.error) {
						// Let user know if their credentials are incorrect
						case "PASSWORD_NOT_CORRECT":
							setError("Incorrect password entered!");
							break;
						case "TOTP_CODE_NOT_CORRECT":
							setError("Invalid 2FA code entered!");
							break;
						case "RECOVERY_CODE_NOT_CORRECT":
							setError("Invalid recovery code entered!");
							break;
						default:
							setError("Internal Server Error");
							break;
					}
				} else {
					// Success message
					if (!r.enabled) {
						setError("");
						setMessage(
							"Two-Factor Authentication has been disabled! You must now login again, redirecting in 5 seconds..."
						);
						// Logout the user after 5 seconds
						setTimeout(() => {
							props.logout();
						}, 5000);
					}
				}
			});
	};
	const enable2FA = () => {
		// enable 2FA using a PUT request
		setError("Please wait...");
		fetch(`/api/verify-totp?code=${totpCode}&secret=${tfaConfig?.secret}`)
			.then((r) => r.json)
			.then((r) => {
				// verify totp code first
				if (!r) {
					setError("Incorrect code entered!");
				} else {
					// PUT request
					fetch(`/api/user/${props.user.user_id}/2fa?password=${password}`, {
						method: "PUT",
						body: JSON.stringify({
							tfa_enabled: true,
							totp_secret: tfaConfig?.secret,
							codes: tfaConfig?.recovery_codes
						})
					})
						.then((s) => s.json())
						.then((s) => {
							if (s.error) {
								// Inform them if credentials are incorrect
								switch (s.error) {
									case "PASSWORD_NOT_CORRECT":
										setError("Incorrect password entered!");
										break;
									default:
										setError("Internal server error");
								}
							} else {
								// success message and logout after 5 seconds
								if (s.enabled) {
									setError("");
									setMessage(
										"Two-Factor Authentication has been enabled! You must now login again, redirecting in 5 seconds..."
									);
									setTimeout(() => {
										props.logout();
									}, 5000);
								}
							}
						});
				}
			});
	};
	const getTfaConfig = () => {
		// generate a random 2FA config
		return fetch(`/api/generate-totp?name=${props.user.email}`)
			.then((r) => r.json())
			.then((r) => setTfaConfig(r));
	};
	const downloadRecoveryCodes = () => {
		//Download the recovery codes as a text file
		setDownloaded(true);
		const element = document.createElement("a");
		const file = new Blob(
			tfaConfig?.recovery_codes.map((r) => r + "\n"),
			{ type: "text/plain" }
		);
		element.href = URL.createObjectURL(file);
		element.download = `boltpass-recovery-codes-${props.user.email}.txt`;
		document.body.appendChild(element);
		element.click();
	};

	if (!tfaConfig && props.user.email) getTfaConfig();
	return (
		<>
			<Logo dark={props.dark} />
			{
				// show the user a different page based on whether they have 2FA enabled or not
				props.user.tfa_enabled ? (
					<>
						<h1>Disable Two Factor Authentication (2FA)</h1>
						<p>
							To disable 2FA please enter your password and either a one-time passcode
							or recovery key below
							<br />
						</p>
						<form
							className="styled-form"
							// allows the user to submit by pressing enter instead of pressing the button
							onSubmit={(e) => {
								e.preventDefault();
								if (
									password &&
									(totpCode.toString().length == 6 ||
										recoveryCode.toString().length == 8)
								) {
									disable2FA();
								}
							}}
						>
							<label className="red">
								Please confirm by entering your password and one of the options below
							</label>
							<input
								type="password"
								placeholder="Enter your password"
								required
								name="password"
								onInput={(v) => setPassword(v.currentTarget.value)}
							/>
							<span className="red">
								You only need to fill in ONE out of the two options below
							</span>
							{/* allow users to use 2FA code or recovery code to delete */}
							<div className="grid">
								<div className="left">
									<label>Enter a one-time passcode</label>
									<input
										type="number"
										placeholder="Enter your OTP"
										minLength={6}
										maxLength={6}
										name="passcode"
										onInput={(v) => setTotpCode(v.currentTarget.value)}
									/>
								</div>
								<div className="right">
									<label>Enter one of your recovery codes</label>
									<input
										type="text"
										placeholder="Enter your recovery code"
										minLength={8}
										maxLength={8}
										name="recovery"
										onInput={(v) => setRecoveryCode(v.currentTarget.value)}
									/>
								</div>
							</div>
							<button
								type="submit"
								// must have entered password and totp code or recovery code
								disabled={
									!password ||
									(totpCode.toString().length != 6 &&
										recoveryCode.toString().length != 8)
								}
							>
								Disable 2FA
							</button>
						</form>

						{error ? <span className="red">{error}</span> : ""}
						{message ? <span className="green">{message}</span> : ""}
					</>
				) : (
					// provides the user a lot of information about why to use 2FA and the process
					<>
						<h1>Enable Two Factor Authentication (2FA)</h1>
						<p>
							2FA provides your account with an additional layer of security by using a
							randomly generated secret key and generating a 6-digit code based on the
							current time. These codes refresh every 30 seconds.
						</p>
						{tfaConfig ? (
							<>
								<div className="grid">
									<div className="left">
										{/* Display the QR code to scan into the authenticator */}
										<h2>Authenticators</h2>
										<p>
											Scan the QR code below in an authenticator app such as Google
											Authenticator or Authy
										</p>
										<div className="image-center">
											<img src={tfaConfig.image} className="qrcode" />
										</div>
										<p>
											Alternatively, you can manually enter the secret key:
											<br />
											Secret: <span>{tfaConfig.secret}</span>
											<br />
											<a
												onClick={async () => {
													await navigator.clipboard.writeText(tfaConfig.secret);
													setCopied(true);
												}}
											>
												<FaCopy /> {copied ? "Copied!" : "Copy"}
											</a>
										</p>
									</div>
									<div className="right">
										{/* user has to download the recovery codes before adding to their account */}
										<h2>Recovery Codes</h2>
										<h3>Please read the following carefully</h3>
										<p>
											If you lose access to your authenticator, the only way to get into
											your account is by using one of these recovery codes.
											<br />
											<span className="red">
												Please store these codes securely as they are the only way to
												recover your account if you lose access to your authenticator.
											</span>
											<br />
											You have been assigned 8 unique recovery codes, and once you have
											used a code, it won't be accepted again. Ensure you re-setup 2FA
											before they have all been used so you don't lose access to your
											account.
											<br />
											You cannot enable 2FA without downloading these codes to your device.
										</p>
										<button onClick={downloadRecoveryCodes}>
											Download Recovery Codes
										</button>
									</div>
								</div>
								<p>
									To add this configuration to your account, please enter your password
									below, and the code generated by your authenticator app
								</p>
								<form
									className="styled-form"
									// allows the user to submit by pressing enter instead of pressing the button
									onSubmit={(e) => {
										e.preventDefault();
										if (password && totpCode.toString().length == 6 && downloaded) {
											enable2FA();
										}
									}}
								>
									{/* must enter password and generated OTP */}
									<label>Enter your password</label>
									<input
										type="password"
										placeholder="Enter your password"
										required
										name="password"
										onInput={(v) => setPassword(v.currentTarget.value)}
									/>
									<label>
										Enter the one-time passcode generated by your authenticator
									</label>
									<input
										type="number"
										placeholder="Enter your OTP"
										minLength={6}
										maxLength={6}
										name="passcode"
										onInput={(v) => setTotpCode(v.currentTarget.value)}
									/>
									<button
										type="submit"
										disabled={!password || totpCode.toString().length != 6 || !downloaded}
									>
										Enable 2FA
									</button>
								</form>

								{!downloaded ? (
									<span className="red">
										You have not downloaded your recovery codes!
									</span>
								) : (
									""
								)}
								{error ? <span className="red">{error}</span> : ""}
								{message ? <span className="green">{message}</span> : ""}
							</>
						) : (
							<p>Please wait while the configuration loads...</p>
						)}
					</>
				)
			}
		</>
	);
}

export default Update2FA;
